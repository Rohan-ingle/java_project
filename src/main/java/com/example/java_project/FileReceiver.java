        ...
        try (ServerSocket serverSocket = new ServerSocket(serverPort)) {
            ...
            try (Socket socket = serverSocket.accept();
                ...
                while (keepConnectionOpen) {
                    ...
                    // Check the command and perform actions accordingly
                    if (commandPacketParts[0].equals("CreateUser")) {
                        // Perform actions for creating a user
                        System.out.println("Creating User");
                    ...
                    } else if (commandPacketParts[0].equals("Logout")) {
                        // Perform actions for updating a user
                        loggedIn = false;
                    } else if (commandPacketParts[0].equals("ReceiveFile") && loggedIn) {
                        // Perform actions for receiving a file
                        FileReceiverFunction fileReceiverFunction = new FileReceiverFunction();

                        fileReceiverFunction.receiveFile(inputStream, commandPacketParts[1], database);
